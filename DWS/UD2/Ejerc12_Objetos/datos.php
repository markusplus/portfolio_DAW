    ...
    <form name="form2" method="post" action="reservas.php">
        ...
        <?php
            if(isset($_GET["num_pasajeros_txt"])) {
                for($i = 0; $i < $_GET["num_pasajeros_txt"]; $i++) {
                    echo "<br><br><label><strong>Pasajero $i:</strong></label><br><br>";
                    echo "<label>Nombre: </label><input type='text' name='nombre_txt'>";
                    echo "<label> Apellidos: </label><input type='text' name='apellidos_txt'><br><br>";
                    echo "<label>Edad: </label><input type='number' name='edad_num' id='edad_num$i' onchange='cambiaEdad($i)'>";
                    echo "<div id='contenedor_dni$i' style='visibility: hidden; display: inline-block'><label> DNI: </label><input type='text' name='dni_txt'></div>";
                }
            }
        ?>
        <br><br>
